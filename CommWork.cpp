		...
		case 2:
			...
			if (checkSize == _packet.data.size) {
				if (check == _packet.data.check) {
					...
					_current.position = -_packet.data.pos / static_cast<double>(1000);
					_current.velocity = -_packet.data.velo / static_cast<double>(1000);
					_current.current = -_packet.data.cur / static_cast <double> (1000);
